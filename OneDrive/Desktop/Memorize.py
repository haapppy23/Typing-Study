...
import warnings
import subprocess 
import random # 👈 문제 섞기를 위한 랜덤 모듈 추가!
...
class MemorizeApp:
    def __init__(self, root):
        self.root = root
        self.root.title("리눅스 마스터 2급 - 최종 합격 암기장 (자유 타이핑 모드)")
    ...
    def load_data(self):
        if not os.path.exists(self.filename):
            messagebox.showerror("에러", "quiz_data.json 파일이 없습니다.")
            self.root.destroy()
            return []
        try:
    ...
    def show_main_menu(self):
        ...
        tk.Label(self.main_frame, text="🐧 리눅스 마스터 2급 빡공 암기장 🐧", font=("Arial", 20, "bold")).pack(pady=20)
        tk.Label(self.main_frame, text="학습할 과목/진도를 선택하세요", font=("Arial", 12)).pack(pady=5)

        categories = [
            ("🔐 [1과목] 권한 및 소유권 관리", 0, 24),
            ("💾 [1과목] 파일 시스템 및 디스크", 24, 52),
            ("🐚 [1과목] 셸(Shell) 및 환경설정", 52, 73),
            ("⚙️ [1과목] 프로세스 및 데몬 관리", 73, 95),
            ("📝 [1과목] 에디터 및 단축키", 95, 114),
            ("📦 [1과목] 패키지 및 압축/컴파일", 114, 133),
            ("🖨️ [1과목] 프린터 및 X 윈도우", 133, 153),
            ("🌐 [2과목] 네트워크 및 프로토콜", 153, 178),
            ("☁️ [2과목] 클러스터 및 기술 표준", 178, 192),
            ("🛠️ [2과목] 종합 명령어 실전 (1)", 192, 223),
            ("🛠️ [2과목] 종합 명령어 실전 (2)", 223, len(self.all_data))
        ]
        ...
        for idx, (title, start, end) in enumerate(categories):
            chunk = self.all_data[start:end]
            if not chunk: continue 
            count = len(chunk)
            
            parent = left_frame if idx < 6 else right_frame
            
            row_frame = tk.Frame(parent)
            row_frame.pack(pady=8, fill="x")
            
            tk.Label(row_frame, text=f"{title} ({count}문제)", font=("Arial", 11, "bold"), width=30, anchor="w").pack(side="left", padx=5)
    ...
    # ==========================================
    # 2. 자유 빡공 모드 (문제 칸 채점 안 함, 순서대로!)
    # ==========================================
    def start_study(self, chunk_data):
        self.current_data = chunk_data # 빡공 모드는 순서대로 진행
        self.current_index = 0
        self.rep_count = 0
    ...
    def start_test(self, chunk_data):
        # 원본 데이터가 꼬이지 않게 리스트를 복사한 뒤, 무작위로 섞어버림!
